...
- 共 43 个案例。
- 40 个第一稿案例：单一客观错误、单一学术表达问题、无明显错误的强文本、多错误混合文本，以及残句、连写句、逗号拼接、论点、衔接、论证和外部测评边界案例。
...
当前 0.2.5 数据集以项目开发阶段的内部案例为主，并纳入了可复现的外部测评案例。进入公开发布门前，仍应由另一名具备学术英语经验的人复核金标准，并根据首轮结果确定正式通过门槛。测试集不能根据某一次模型输出反向修改；新增案例必须先有独立语言判断与可复现输入。
