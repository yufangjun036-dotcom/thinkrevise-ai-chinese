...
Production deployment `98ebc9b` completed successfully. Direct checks against `https://thinkrevise-ai-chinese.vercel.app/` returned the live OpenAI provider and passed the three strict boundary cases: 1 language + 5 register findings for the evaluator article, zero findings for the correct control, and 0 language + 1 evidence finding for the grammar-corrected overclaim. The final production first-to-second-draft journey returned 9 language + 1 academic item initially, then 0 language + 1 retained academic item, with 9 resolved, 1 changed-location, and 0 supplemental findings. One request made immediately after deployment completion still returned the preceding eight-item behaviour during alias propagation; it was not counted as a pass, and subsequent production requests confirmed the new deterministic rules.
