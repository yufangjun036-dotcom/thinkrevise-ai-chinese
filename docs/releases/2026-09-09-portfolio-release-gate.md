...
After commit `54ffaef` deployed, the same synthetic article was restarted from a fresh production page. The initial diagnosis returned only the three intended language issues (`teh`, `students is`, and `checking the answer careful`). After all three were corrected, the second-draft comparison reported 3 no longer detected, 0 remaining, 0 changed-location and 0 supplemental findings. The final version matched the learner's corrected second draft exactly, so the duplicated conclusion did not recur.
